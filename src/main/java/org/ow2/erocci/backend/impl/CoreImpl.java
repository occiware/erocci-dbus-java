...
public class CoreImpl implements core, DBus.Properties {
	 ...
	 * 
	 * @param location
	 *            (string): entity path relative part
	 * @param kind
	 *            (string): kind id
	 * @param mixins
	 *            (string array): mixins ids
	 * @param attributes
	 *            (string/variant array): entity attributes
	 * @param owner
	 *            (string): entity owner (empty=anonymous)
	 * @param group
	 *            (string): entity group (empty=anonymous), for now this is
	 *            ignored.
	 ...
	 *            (string): entity owner (empty=anonymous)
	 * @param group
	 *            (string): entity group (empty=anonymous)
	 * @return
	 */
	@Override
	...
	public Septuple<String, List<String>, Map<String, Variant>, List<String>, String, String, String> Get(
		...
		if (entity != null) {
			...
			List<String> mixinsToReturn = new ArrayList<String>();
	...
	// */
	// @Override
	// public String SaveResource(String id, String kind, java.util.List<String>
	// mixins, Map<String, Variant> attributes,
	// String owner) {
	// LOGGER.info("Save resource input with id=" + id + ", kind=" + kind + ",
	// mixins=" + mixins + ", attributes="
	...
	// // if necessary).
	// if (ConfigurationManager.isEntityExist(owner, entityId)) {
	// LOGGER.info("Overwrite resource invoked with id=" + id + ", kind=" + kind
	// + ", mixins=" + mixins
	// + ", attributes=" + Utils.convertVariantMap(attributes));
	// ConfigurationManager.addResourceToConfiguration(id, kind, mixins, attr,
	// owner);
	// } else {
	// LOGGER.info("SaveResource invoked with id=" + entityId + ", kind=" + kind
	// + ", mixins=" + mixins
	// + ", attributes=" + Utils.convertVariantMap(attributes));
	// attr.put("occi.core.id", entityId);
	// ConfigurationManager.addResourceToConfiguration(entityId, kind, mixins,
	// attr, owner);
	// }
	// Entity entity = ConfigurationManager.findEntity(owner, entityId);
	// if (entity != null) {
	...
	// } else {
	// LOGGER.error("Error, entity was not created on object model, please check
	// your query.");
	// throw new RuntimeException("Error, entity was not created on object
	// model, please check your query.");
	// }
	//
	//
	// return id;
	...
	//
	// /**
	// * Record version 1 of a link (overwrite mode: use Update instead). If
	// links
	...
	// */
	// @Override
	// public String SaveLink(String id, String kind, java.util.List<String>
	// mixins, String src, String target,
	...
	// // if necessary).
	// if (ConfigurationManager.isEntityExist(owner, entityId)) {
	// LOGGER.info("Overwrite link invoked with id=" + id + ", kind=" + kind +
	// ", mixins=" + mixins
	// + ", attributes=" + Utils.convertVariantMap(attributes));
	// ConfigurationManager.addLinkToConfiguration(id, kind, mixins, src,
	// target, attr, owner);
	//
	// } else {
	// LOGGER.info("SaveLink invoked with id=" + entityId + ", kind=" + kind +
	// ", mixins=" + mixins
	// + ", attributes=" + Utils.convertVariantMap(attributes));
	//
	// attr.put("occi.core.id", entityId);
	//
	// ConfigurationManager.addLinkToConfiguration(entityId, kind, mixins, src,
	// target, attr, owner);
	//
	// }
	//
	...
	// */
	// @Override
	// public Map<String, Variant> Update(String id, Map<String, Variant>
	// attributes) {
	...
	// }
	// if (entity == null) {
	// LOGGER.error("entity : " + id + " has not been found for update, cant
	// update.");
	// throw new RuntimeException("entity : " + id + " has not been found for
	// update, cant update.");
	// } else {
	...
	// ConfigurationManager.findEntitiesOnAllOwner(id);
	// Entity ent;
	// String owner;
	//
	// if (!entities.isEmpty()) {
	...
	// ret.add(new Struct1(CoreImpl.NODE_ENTITY, new
	// Variant<String>(ent.getId()), owner,
	...
	//
	// // ConfigurationManager.printEntity(ent);
	// ret.add(new Struct1(CoreImpl.NODE_UNBOUNDED_COLLECTION, new
	// Variant<String>(ent.getId()), owner,
	// ConfigurationManager.getEtagNumber(owner, id)));
	// }
	// }
	...
	// */
	// @Override
	// public Quad<String, String, java.util.List<String>, Map<String, Variant>>
	// Load(Variant opaque_id) {
	...
	// * @param filters
	// * @return Pair, where Variant<String> is the iterator ID for Next()
	// method,
	// * and UInt32 the iterator's serial num (eg. for caching).
	// */
	// @Override
	// public Pair<Variant, UInt32> List(String id, Map<String, Variant>
	// filters) {
	// LOGGER.info("List invoked with id=" + id + " and filters=" + filters);
	// // TODO : Next step, Add support for root query like :
	// http://localhost:8080/ <--< "/".
	...
	// return new Pair<Variant, UInt32>(new Variant<String>(collectionName), new
	// UInt32(collectionNb));
	// }
	//
	// /**
	...
	// UInt32 items) {
	// LOGGER.info("Next invoked with opaque_id=" + opaque_id + ", start=" +
	// start + ", items=" + items);
	// List<Struct2> fullList =
	// currentListRequests.remove(opaque_id.getValue());
	// if (fullList != null) {
	...
	// *
	// * @param id if full url: category id (bounded collection) if path
	// relative
	...
	// ConfigurationManager.findAllEntitiesLikePartialId(ConfigurationManager.DEFAULT_OWNER,
	// id);
	...
	// // We consider here that this is by a location
	// (http://localhost:8080/myxinsCollection/).
	// Map<String, List<String>> userMixins =
	// ConfigurationManager.findAllUserMixinKindByLocation(id);
	// List<String> mixins;
	...
	// // id may be a relative part of a mixin kind. Note that the method use
	// contains for replacing equality on location.
	// Map<String, List<String>> userMixins =
	// ConfigurationManager.findAllUserMixinKindByLocation(id);
	// List<String> mixins;
	// for (Map.Entry<String, List<String>> entry : userMixins.entrySet()) {
	...
	// */
	// @Override
	// public void Action(String id, String action_id, Map<String, Variant>
	// attributes) {
